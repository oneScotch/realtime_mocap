...
def convert_impl(d, src_rep, dst_rep):
    ...
    if src_rep == 'rot6d':
        ...
        rep_pair2f = {
            ('aa', 'rotmtx'): kornia.geometry.conversions.angle_axis_to_rotation_matrix,
            ('aa', 'quat'): kornia.geometry.conversions.angle_axis_to_quaternion,
            ('rotmtx', 'aa'): kornia.geometry.conversions.rotation_matrix_to_angle_axis,
            ('rotmtx', 'quat'): kornia.geometry.conversions.rotation_matrix_to_quaternion,
            ('quat', 'aa'): kornia.geometry.conversions.quaternion_to_angle_axis,
            ('quat', 'rotmtx'): kornia.geometry.conversions.quaternion_to_rotation_matrix
        }
...
def rep_reshape_forward(d, src_rep, dst_rep, multiple=False):
    ...
    check_rep(dst_rep)
    orig_shape = d.shape
    shape, cache = rep_shape_forward(
        orig_shape, src_rep, dst_rep, multiple=multiple)
...
def rep_shape_backward(cache):
    ...
    if non_equal_shape_index == -1:
        ...
        dst_shape = list(
            orig_shape[:-1]) + [int(count * np.prod(dst_rep_shape))]
    return dst_shape


def rep_reshape(shape, src_rep, dst_rep, multiple=False):
    shape, cache = rep_shape_forward(
        shape, src_rep, dst_rep, multiple=multiple)
...
def is_valid_rotmat(rotmats, thresh=1e-6):
    ...
    rotmats_t = np.transpose(rotmats,
                             tuple(range(len(rotmats.shape[:-2]))) + (-1, -2))
...
def rotmat2euler(rotmats):
    ...
    found here: https://github.com/una-dinosauria/human-motion-
    prediction/blob/master/src/data_utils.py#L12.
