...
from realtime_mocap.extern.patched_smplx.patched_smplx.utils import to_np
from scipy.special import softmax
from .angle_representation import universe_convert
from realtime_mocap.extern.smplx_optimization.smplx_optimization.pykinect.pose_init import initialize_pose_advanced
from realtime_mocap.extern.smplx_optimization.smplx_optimization.pykinect.smplx_model import ExpBodyModel
...
def load_exp_bm(pykinect_data_dp, gender, n_pca, device):
    if gender == 'male':
        bm_path = osp.join(pykinect_data_dp,
    ...
    kinect_vert_weights_path = osp.join(pykinect_data_dp,
...
def inf_exp_bm(exp_bm,
    ...
    exp_bm_out = exp_bm(global_rot, pose_body, pose_hand, pose_jaw, pose_eyes,
                        beta, global_trans, face_expression)
    return exp_bm_out


def exp_bm_out2kinect_joints(exp_bm_out,
                             substract_pelvis=False,
...
def calc_kinect_twists(kinect_joints, init_kinect_joints, a_inv, bones):
    ...
    kinect_bones = torch.from_numpy(kinect_joints[bones[:, 0]] -
                                    kinect_joints[bones[:, 1]]).type(
                                        torch.float32)
    kinect_bones = torch.bmm(kinect_bones.unsqueeze(1),
                             a_inv[bones[:, 0]]).squeeze(1)

    init_kinect_bone = torch.from_numpy(init_kinect_joints[bones[:, 0]] -
                                        init_kinect_joints[bones[:, 1]]).type(
                                            torch.float32)
    init_kinect_bone = torch.bmm(
        init_kinect_bone.unsqueeze(1), a_inv[bones[:, 0]]).squeeze(1)

    kinect_twists = rotate_a_b_axis_angle_torch_batched(
        kinect_bones, init_kinect_bone)
...
class ExpBMWrapper:
    ...
    def __init__(self,
                 ...
                 device,
                 n_pca=12,
                 override_kinect2smplx_mapping=True):
        ...
        if override_kinect2smplx_mapping:
            self.bodyparts = np.array([
                0, 3, 6, 9, 13, 16, 18, 20, 20, 20, 20, 14, 17, 19, 21, 21, 21,
                21, 1, 4, 7, 10, 2, 5, 8, 11, 12, 15, 15, 15, 15, 15
            ])
        else:
            bp_weights_dp = osp.join(self.pykinect_data_dp,
                                     'rob75_val/weights.npy')
        ...
        for gender in ['female', 'male']:
            exp_bm, s2k, J, kintree_table = load_exp_bm(
                self.pykinect_data_dp, gender, n_pca, device)
            exp_bm.to(device)
    ...
    def inf_exp_bm(self,
        ...
        exp_bm_out = inf_exp_bm(
            exp_bm=self.pykinect_data[gender]['exp_bm'],
            device=self.device,
    ...
    def get_twists_v2(self, init_A, init_kinect_joints, target_kinect_joints):
        ...
        init_A_select = torch.index_select(init_A, 0,
                                           torch.LongTensor(self.bodyparts))
        init_A_select_inv = torch.transpose(init_A_select, -2, -1)

        init_dirs = init_kinect_joints[self.bones[:, 1]] - init_kinect_joints[
            self.bones[:, 0]]
        ...
        target_dirs = target_kinect_joints[
            self.bones[:, 1]] - target_kinect_joints[self.bones[:, 0]]
    ...
    def get_twists(self, A, target_kinect_joints, init_kinect_joints):
        ...
        a = torch.index_select(
            A.squeeze(0), 0, torch.LongTensor(self.bodyparts))
        a_inv = torch.inverse(a[:, :3, :3])

        kinect_twists = calc_kinect_twists(target_kinect_joints,
                                           init_kinect_joints, a_inv,
                                           self.bones)

        kinect_twists = universe_convert(
            np.array(kinect_twists), 'aa', 'rotmtx').reshape(31, -1)
