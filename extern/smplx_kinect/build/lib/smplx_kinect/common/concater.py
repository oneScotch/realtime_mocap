...
def features_axis(d, is_seq=False):
    ...
    :return: axis of features
    """
    return features_axis_is_np(isinstance(d, np.ndarray), is_seq=is_seq)
...
def universe_len(d, is_seq=False):
    if isinstance(d, NamedDataConcater):
        ...
        return universe_len_is_np(
            d.shape, isinstance(d, np.ndarray), is_seq=is_seq)
...
def universe_get_slice(d, slc, is_seq=False):
    if isinstance(d, NamedDataConcater):
        ...
        if isinstance(d, np.ndarray):
            ...
            return torch.narrow(
                d, dim=ax, start=slc.start, length=slc.stop - slc.start)
...
class NamedDataConcater:
    ...
    def __init__(self,
        ...
        if named_slice_hierarchy is not None:
            ...
            self.named_slice_hierarchy = NamedSliceHierarchy(
                slice(0, len(self)), name, **kwargs)
    ...
    def name2data(self, name):
        ...
        return universe_get_slice(
            self.data, sl, is_seq=self.is_seq) if sl is not None else None
    ...
    def concat(named_data_concaters, name=None):
        ...
        data = universe_concat([ndc.data for ndc in named_data_concaters],
                               is_seq=ndc0.is_seq)
        result = NamedDataConcater(
            data=data, name=name, is_seq=named_data_concaters[0].is_seq)
