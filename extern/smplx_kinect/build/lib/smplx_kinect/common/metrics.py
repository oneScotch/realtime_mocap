...
import numpy as np
import operator
from .angle_representation import (
    get_closest_rotmat,
    is_valid_rotmat,
    local_rot_to_global,
    rotmat2euler,
...
def angle_diff(predictions, targets):
    """Computes the angular distance between the target and predicted
    rotations. We define this as the angle that is required to rotate one
    rotation into the other. This essentially computes || log(R_diff) || where
    R_diff is the difference rotation between prediction and target.
...
class MetricsEngine:
    ...
    def __init__(self,
        ...
        if pck_thresholds is None:
            pck_thresholds = [
                0.001, 0.005, 0.01, 0.02, 0.05, 0.1, 0.15, 0.2, 0.3
            ]
        self.pck_thresholds = pck_thresholds
        self.pck_thresholds_str = [
            f'pck_{pck_threshold}' for pck_threshold in pck_thresholds
        ]
    ...
    def from_rotmat(self, pose_all):
        ...
        for b in range(B):
            if jaw_pose is not None:
                jaw_pose = universe_convert(
                    jaw_pose[b, :], 'rotmtx', 'aa', multiple=True)
            model_output = self.smplx_wrapper.get_output(
                gender='male',
                betas=np.zeros(10),
                body_pose=universe_convert(
                    pose[b, 9:], 'rotmtx', 'aa', multiple=True),
                rvec=universe_convert(
                    pose[b, :9], 'rotmtx', 'aa', multiple=True),
                tvec=np.zeros(3),
                jaw_pose=jaw_pose)
    ...
    def compute(self, predictions, targets, reduce_fn='mean'):
        ...
        """
        assert len(predictions.shape) == len(
            targets.shape), f'{predictions.shape} != {targets.shape}'
        for i in range(len(predictions.shape)):
            assert predictions.shape[i] == targets.shape[
                i], f'{predictions.shape} != {targets.shape}'
        assert predictions.shape[
            -1] % self.dof == 0, 'predictions are not rotation matrices'
        assert targets.shape[
            -1] % self.dof == 0, 'targets are not rotation matrices'
        ...
        pred_are_valid = is_valid_rotmat(
            np.reshape(pred, [-1, n_joints, 3, 3]))
        assert pred_are_valid, 'predicted rotation matrices are not valid'
        targ_are_valid = is_valid_rotmat(
            np.reshape(targ, [-1, n_joints, 3, 3]), thresh=1e-5)
        ...
        for metric in self.which:
            if metric.startswith('pck'):
                ...
                v = pck(
                    pred_pos[:, select_joints],
                    targ_pos[:, select_joints],
                    thresh=thresh)  # (-1, )
                metrics[metric] = np.reshape(v, [batch_size, seq_length])
            elif metric == 'positional':
                v = positional(pred_pos[:, select_joints],
                               targ_pos[:, select_joints])  # (-1, n_joints)
                ...
                pred_global = local_rot_to_global(
                    pred, self.parents, left_mult=self.left_mult,
                    rep='rotmat')  # (-1, full_n_joints, 3, 3)
                targ_global = local_rot_to_global(
                    targ, self.parents, left_mult=self.left_mult,
                    rep='rotmat')  # (-1, full_n_joints, 3, 3)
                v = angle_diff(pred_global[:, select_joints],
                               targ_global[:, select_joints])  # (-1, n_joints)
                v = np.reshape(v, [batch_size, seq_length, n_joints])
                metrics[metric] = reduce_fn_np(v, axis=-1)
            elif metric == 'euler':
                ...
                v = euler_diff(pred_local[:, select_joints],
                               targ_local[:, select_joints])  # (-1, )
                metrics[metric] = np.reshape(v, [batch_size, seq_length])
            else:
    ...
    def get_summary_metrics(self, target_lengths, at_mode=True):
        ...
        for seq_length in sorted(target_lengths):
            ...
            for k, v in sorted(
                    summary_metrics.items(), key=operator.itemgetter(0)):
    ...
    def get_summary_metrics_str(self, summary_metrics):
        s = ''
        for seq_length, seq_length_result in sorted(
                summary_metrics.items(), key=operator.itemgetter(0)):
            s += '\nMetrics until {:<2}:'.format(seq_length)
            for k, v in sorted(
                    seq_length_result.items(), key=operator.itemgetter(0)):
                s += '   {}: {:.3f}'.format(k, v)
        return s
