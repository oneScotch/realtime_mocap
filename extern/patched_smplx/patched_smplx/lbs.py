...
def find_dynamic_lmk_idx_and_bcoords_projective(vertices,
                                                neck_joint_loc,
                                                pose,
    ...
    rel_rot_mat = torch.eye(
        3, device=vertices.device, dtype=dtype).reshape(1, 3,
                                                        3).repeat(b, 1, 1)
    for idx in range(len(neck_kin_chain)):
        rel_rot_mat = torch.bmm(rot_mats[:, idx], rel_rot_mat)
    neck_cam_loc = -torch.bmm(
        torch.transpose(rel_rot_mat, 1, 2), neck_joint_loc.reshape(
            -1, 3, 1)).reshape(-1, 3)
    neck_cam_loc_len = torch.norm(
        neck_cam_loc, dim=1).reshape(-1, 1).repeat(1, 3)
    ...
    y_rot_angle = torch.round(torch.clamp(y_ang * 180.0 / np.pi,
...
def lbs(betas,
    ...
    if pose2rot:
        ...
        pose_offsets = torch.matmul(
            pose_feature.view(batch_size, -1),
...
def vertices2joints_np(J_regressor_np, vertices_np):
    ...
    return np.matmul(
        np.tile(J_regressor_np.reshape(1, n_j, n_v), (b, 1, 1)), vertices_np)
...
# @jit
def batch_rigid_transform_diff(rot_mats,
                               rot_mats_jac,
                               transform_jac_chain,
                               joints,
                               parents,
    ...
    if is_jac:
        ...
        transform_jac_chain[:, ainds, ainds, :, 0:3, 0:3] = rot_mats_jac[:,
                                                                         ainds]

    transform_chain = np.zeros((b, parents.shape[0], 4, 4),
    ...
    if is_jac:
        ...
        trans_parent = transform_chain[:, parents[inds]].reshape(
            b, len(inds), 1, 1, 4, 4)
        trans_jac = transform_jac_chain[:, inds, inds, :].reshape(
            1, len(inds), 1, 3, 4, 4)
        transform_jac_chain[:, inds, inds, :] = np.matmul(
            trans_parent, trans_jac).reshape(len(inds), 3, 4, 4)
        ...
        for i in range(1, parents.shape[0]):
            ...
            transform_jac_chain[:, i, m[i], :] += np.matmul(
                tr_jac_ch_sel, transforms_mat[:, i]).reshape(b, -1, 3, 4, 4)

    t_2 = time.time()

    transforms = transform_chain
    posed_joints = transforms[:, :, :3, 3]
    joints_rot = np.matmul(transforms[:, :, 0:3, 0:3],
                           joints.reshape(b, n_j, 3, 1)).reshape((b, n_j, 3))
    ...
    if is_jac:
        ...
        tjhj = np.matmul(transforms_jac[:, :, :, :, 0:3, 0:3],
                         joints.reshape((b, 1, 1, n_j, 3, 1))).reshape(
                             (b, n_j, 3, n_j, 3))
        rel_transforms_jac = transforms_jac.copy()
        rel_transforms_jac[:, :, :, :, 0:3,
                           3] = rel_transforms_jac[:, :, :, :, 0:3, 3] - tjhj
    else:
...
def batch_rigid_transform_fast_diff(rot_mats, rot_mats_jac, joints, parents):
    ...
    for i in range(1, parents.shape[0]):
        t_curr = np.matmul(transform_chain[:, parents[i], 0:3, 0:3],
                           transforms_mat[:, i, 0:3,
                                          3].reshape(-1, 1, 3,
                                                     1)).reshape(-1, 1, 3)
        transform_chain[:, i, 0:3,
                        3] = t_curr + transform_chain[:, parents[i], 0:3, 3]
        t_i = np.tile(transforms_mat[:, i, 0:3, 3].reshape(-1, 1, 1, 3, 1),
                      (1, n_j, 3, 1, 1))
        # print(transform_jac_chain[:, :, :, parents[i], 0:3, 0:3].shape)
        # print(t_i.shape)
        t_jac_curr = np.matmul(
            transform_jac_chain[:, :, :, parents[i], 0:3, 0:3], t_i)
...
def lbs_diff_fast(pose,
    ...
    J_transformed, J_transformed_jac, A, A_jac = batch_rigid_transform_fast_diff(
        rot_mats, rot_mat_jacs, J, parents)
...
def lbs_diff(pose,
    ...
    if v_inds is not None:
        v_shaped = v_shaped[:, v_inds, :]
        inds = np.stack([3 * v_inds, 3 * v_inds + 1, 3 * v_inds + 2],
                        axis=1).reshape(-1)
    ...
    for i in range(1, n_j):
        pdi = np.matmul(rot_mat_jacs[:, i].reshape(-1, 9),
                        posedirs[(i - 1) * 9:i * 9, :]).reshape(
                            (batch_size, 3, -1, 3))
    ...
    J_transformed, J_transformed_jac, A, A_jac = batch_rigid_transform_diff(
        rot_mats, rot_mat_jacs, transform_jac_chain, J, parents)
    ...
    v_homo_jac1 = np.matmul(
        T_j[:, :, :, :, 0:3, 0:3],
        v_posed_jac.reshape((batch_size, n_j, 3, n_v, 3, 1)))
...
def lbs_diff_nopd(pose,
    ...
    if v_inds is not None:
        ...
        if face_expression is not None:
            ...
            n_v = len(v_inds)
            dv = np.matmul(
                posedirs_face.reshape(1, n_v, 3, 1, -1),
                np.tile(
                    face_expression.reshape(1, 1, 1, -1, 1),
                    (1, n_v, 3, 1, 1)))
            v_shaped += dv.reshape(1, -1, 3)
            dv_jac = np.transpose(
                posedirs_face.reshape(n_v, 3, -1),
                (2, 0, 1)).reshape(-1, 3 * n_v)
    ...
    J_transformed, J_transformed_jac, A, A_jac = batch_rigid_transform_diff(
        rot_mats, rot_mat_jacs, transform_jac_chain, J, parents, is_jac)
    ...
    if is_jac:
        ...
        T_jac = np.matmul(
            W_j,
            A_jac.reshape(batch_size, n_j, 3, n_j, -1)[:, :, :, bpm])
        T_jac = T_jac.reshape((batch_size, n_j, 3, n_v, 4, 4))
        ...
        if face_expression is not None:
            n_fp = 10
            dv_jac = np.matmul(T[:, :, 0:3, 0:3],
                               dv_jac.reshape(n_fp, -1, 3,
                                              1)).reshape(n_fp, -1)
