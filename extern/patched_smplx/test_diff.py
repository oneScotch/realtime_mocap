...
import torch
import trimesh
from smplx.body_models import SMPL, SMPLH, SMPLX
from smplx.lbs import (
    batch_rigid_transform_diff,
    batch_rodrigues_np,
    lbs_diff,
    lbs_diff_fast,
    lbs_diff_nopd,
...
def fun1(batch_size, n_j, pose_body, J, parents):
    ...
    return batch_rigid_transform_diff(rot_mats, rot_mat_jacs,
                                      transform_jac_chain, J, parents)
...
def test_batch_rigid_transform_diff(data_path):
    ...
    v_template = np.repeat(
        smpl_dict['v_template'][np.newaxis], batch_size, axis=0)
    ...
    body_pose = torch.tensor(
        pose[:, 3:3 * (SMPLX.NUM_BODY_JOINTS + 1)], dtype=torch.float)
    lh_pose = torch.tensor(
        pose[:, 3 * (SMPLX.NUM_BODY_JOINTS + 1):3 *
             (SMPLX.NUM_BODY_JOINTS + SMPLX.NUM_HAND_JOINTS + 1)],
        dtype=torch.float)
    ...
    J, v_shaped, homogen_coord, transform_jac_chain = prepare_J(
        shape_components, v_template, shapedirs, J_regressor, n_v)

    # 3. Add pose blend shapes
    # N x J x 3 x 3
    pose = np.zeros((batch_size, n_j * 3))
    J_transformed, J_transformec_jac, A, A_jac = fun1(batch_size, n_j, pose, J,
                                                      parents)
    ...
    for rot_id in range(0, n_j):
        for i in range(0, 3):
            ...
            J_transformed_p, J_transformec_jac_p, A_p, A_jac_p = fun1(
                batch_size, n_j, pose_p, J, parents)
...
def test_lbs_diff(data_path):
    ...
    verts, verts_jac, J_transformed, J_transformed_jac, A, A_jac, J = lbs_diff(
        pose, posedirs, parents, J, v_shaped, weights, homogen_coord,
        transform_jac_chain, v_inds)
...
def test_lbs_diff_nopd():
    ...
    verts, verts_jac, J_transformed, J_transformed_jac, A, A_jac, J, bpm = lbs_diff_nopd(
        pose, posedirs, parents, J, v_shaped, weights, homogen_coord,
        transform_jac_chain, v_inds)
        ...
        for i in range(0, 3):
            ...
            if v_err > 1e-5:
                for v_id in range(0, dV.shape[1]):
                    if (np.linalg.norm(
                            dV_pred.reshape(-1, 3)[v_id] - dV[0, v_id]) >
                            1e-3):
...
def test_lbs_diff_faceexpr_nopd(data_path):
    ...
    for i in range(0, 10):
        ...
        if v_err > 1e-5:
            for v_id in range(0, dV.shape[1]):
                if (np.linalg.norm(dV_pred.reshape(-1, 3)[v_id] - dV[0, v_id])
                        > 1e-3):
                    print(dV_pred.reshape(-1, 3)[v_id])
                    print(dV[0, v_id])
                    print('-')
...
def test_lbs_diff_fast():
    ...
    J, v_shaped, W, W_j, homogen_coord = prepare_J(shape_components,
                                                   v_template, shapedirs,
                                                   J_regressor, weights,
                                                   v_inds)

    pose_direct = rel_to_direct(pose, parents)
    verts, verts_jac, J_transformed, J_transformed_jac, A, A_jac, J = lbs_diff_fast(
        pose_direct, parents, J, v_shaped, W, W_j, homogen_coord, v_inds)
