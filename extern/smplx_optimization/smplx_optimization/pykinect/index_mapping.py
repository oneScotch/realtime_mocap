...
def get_smplx_ids_op_face():
    inds_76_144 = np.concatenate([np.arange(51, 68), np.arange(0, 51)]) + 76
...
def get_smplx_ids_op_lh():
    ...
    return np.asarray(
        ind_root + inds_big + inds_1 + inds_2 + inds_3 + inds_little,
        dtype=int)
...
def get_kinect_smplx_vert():
    return np.asarray(
        [[15, 7353, 21], [16, 7795, 21], [17, 8099, 21], [8, 4826, 20],
         [9, 5082, 20], [10, 5365, 20], [20, 5877, 7], [21, 5895, 10],
         [24, 8571, 8], [25, 8589, 11]],
        dtype=int)
...
def get_vino_static_verts():
    ...
    # print(vino_vs)
    vino_vs = np.asarray([
        2485, 2441, 1243, 1176, 8970, 8996, 2747, 1604, 2821, 9372, 8985, 8947,
        9215, 335, 9240, 9364, 16, 694
    ],
                         dtype=int)
    return vino_vs
...
def get_face_smplx_landmarks():
    ...
    fl = np.asarray(
        [[12, 77], [14, 80], [15, 81], [17, 84], [5, 92], [6, 90], [7, 94],
         [4, 89], [10, 110], [11, 116], [8, 107], [9, 113]],
        dtype=int)
