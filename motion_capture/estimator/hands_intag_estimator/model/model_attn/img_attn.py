...
class img_feat_to_grid(nn.Module):
    ...
    def __init__(self,
        ...
        self.position_embeddings = nn.Embedding(grid_size * grid_size,
                                                grid_f_dim)

        patch_size = img_size // grid_size
        self.proj = nn.Conv2d(
            img_f_dim, grid_f_dim, kernel_size=patch_size, stride=patch_size)
        self.self_attn = SelfAttn(
            grid_f_dim, n_heads=n_heads, hid_dim=grid_f_dim, dropout=dropout)

    def forward(self, img):
        # img.shape: [1, 256, 8, 8]
        # img.shape: [1, 256, 16, 16]
        # img.shape: [1, 256, 32, 32]
        bs = img.shape[0]
        ...
        # position_embeddings.shape: [1, 64, 256]
        # position_embeddings.shape: [1, 64, 128]
        # position_embeddings.shape: [1, 64, 64]

        grid_feat = F.relu(self.proj(img))
        # grid_feat.shape: [1, 256, 8, 8]
        # grid_feat.shape: [1, 128, 8, 8]
        # grid_feat.shape: [1, 64, 8, 8]
...
class img_attn(nn.Module):
    ...
    def __init__(self,
        ...
        if self.verts_f_dim == 256:
            self.verts_shape_1 = 63
        elif self.verts_f_dim == 128:
            self.verts_shape_1 = 126
        elif self.verts_f_dim == 64:
            self.verts_shape_1 = 252
        else:
            raise RuntimeError
...
class img_ex(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.verts_f_dim = verts_f_dim
        self.encoder = img_feat_to_grid(img_size, img_f_dim, grid_size,
                                        grid_f_dim, n_heads, dropout)
        self.attn = img_attn(
            verts_f_dim, grid_f_dim, n_heads=n_heads, dropout=dropout)
