...
class DualGraphLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.position_embeddings = nn.Embedding(self.verts_num,
                                                self.verts_in_dim)

        self.graph_left = GraphLayer(verts_in_dim, verts_out_dim, graph_L_Left,
                                     graph_k, graph_layer_num, dropout)
    ...
    def forward(self, Lf, Rf, img_f):
        ...
        position_ids = torch.arange(
            self.verts_num, dtype=torch.long, device=Lf.device)
        position_ids = position_ids.unsqueeze(0).repeat(BS, 1)
        position_embeddings = self.position_embeddings(position_ids)
        Lf = Lf + position_embeddings
        Rf = Rf + position_embeddings

        # Lf.shape before graph_left: [1, ?, 512 or 256 or 128]
        Lf = self.graph_left(Lf)
        Rf = self.graph_right(Rf)
        # Lf.shape after graph_left: [1, ?, 256 or 128 or 64]

        # Lf shape before and after self.img_ex_left: [1, 63, 256]
...
class DualGraph(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(verts_in_dim) - 1):
            ...
            assert graph_L_Right[i +
                                 1].shape[0] == 2 * graph_L_Right[i].shape[0]
        ...
        for i in range(len(verts_in_dim)):
            self.layers.append(
                DualGraphLayer(
                    ...
                    dropout=dropout))
        # [1, 256, 63] -> [1, 256, 126]
        self.up_sam_63 = nn.Upsample(size=(126, ))
        # [1, 128, 126] -> [1, 128, 252]
        self.up_sam_126 = nn.Upsample(size=(252, ))
    ...
    def forward(self, Lf, Rf, img_f_list):
        ...
        for i in range(len(self.layers)):
            ...
            if i != len(self.layers) - 1:
                # Lf.shape before upsampling: [1, 63 or 126, ?]
                if Lf.shape[1] == 63:
                    Lf = self.up_sam_63(Lf.permute(0, 2,
                                                   1).contiguous()).permute(
                                                       0, 2, 1).contiguous()
                    Rf = self.up_sam_63(Rf.permute(0, 2,
                                                   1).contiguous()).permute(
                                                       0, 2, 1).contiguous()
                elif Lf.shape[1] == 126:
                    Lf = self.up_sam_126(Lf.permute(0, 2,
                                                    1).contiguous()).permute(
                                                        0, 2, 1).contiguous()
                    Rf = self.up_sam_126(Rf.permute(0, 2,
                                                    1).contiguous()).permute(
                                                        0, 2, 1).contiguous()
                else:
                    raise RuntimeError
                ...
                # Lf.shape after upsampling: [1, 126 or 252, ?]
